...
UnsortedArrayDemoApp::UnsortedArrayDemoApp()
	: SubApp(EAppID::UnsortedArrayDemoApp),
	m_List()
{}

void UnsortedArrayDemoApp::Run()
{
...
void UnsortedArrayDemoApp::Restart()
{}

void UnsortedArrayDemoApp::Clean()
{
	m_List.clear();
}

void UnsortedArrayDemoApp::HandleInput(char p_Input)
{
	switch (p_Input)
	{
	case 'A':
	...
	case 'B':
	{
		...
		printf("\n\t\"%s\" was successfully pushed into the array!", str.c_str());
	}
		break;
	case 'C':
		puts("");
	...
	case 'D':
	{
		...
		if (iter == m_List.end())
		{
			printf("\"%s\" is not found in the unsorted array!", str.c_str());
			break;
		}

		size_t index = std::distance(m_List.begin(), iter);
		printf("\n\t\"%s\" is found in the unsorted array at index [%d]!", str.c_str(), index);
	}
		break;
	case 'E':
		m_List.clear();
